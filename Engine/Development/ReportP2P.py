...
spark = SparkSession.builder.appName("PathToPurchaseReport").getOrCreate()
...
def count_deterministic_pids_per_domain():
    count_deterministic_pids_per_domain = self.data.filter(self.data["name"] == "count_deterministic_pids_per_domain") \
        .select("name", "domain", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
    nok_data = count_deterministic_pids_per_domain.filter(count_deterministic_pids_per_domain["verdict"] != "OK") \
...
def count_nondeterministic_pids_per_domain():
    count_nondeterministic_pids_per_domain = self.data.filter(self.data["name"] == "count_nondeterministic_pids_per_domain") \
        .select("name", "domain", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
    nok_data = count_nondeterministic_pids_per_domain.filter(count_nondeterministic_pids_per_domain["verdict"] != "OK") \
        .orderBy(F.desc("difference"))
    metric_wrapper = metric_wrapper_func(func.__name__)
    results, legend = per_domain_results(nok_data)
    insert_position = metric_wrapper.find("</div>")
...
def count_rows_per_dataset():
    count_rows_per_dataset = self.data.filter(self.data["name"] == "count_rows_per_dataset") \
        .select("name", "domain", "dataset", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
    nok_data = count_rows_per_dataset.filter(count_rows_per_dataset["verdict"] != "OK") \
        .orderBy(F.desc("difference"))
    metric_wrapper = metric_wrapper_func(func.__name__)
...
def count_rows_per_domain():
    count_rows_per_domain = self.data.filter(self.data["name"] == "count_rows_per_domain") \
        .select("name", "domain", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
...
def count_rows_per_metadatalink_per_domain():
    count_rows_per_metadatalink_per_domain = self.data.filter(self.data["name"] == "count_rows_per_metadatalink_per_domain") \
        .select("name", "domain", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
    nok_data = count_rows_per_metadatalink_per_domain.filter(count_rows_per_metadatalink_per_domain["verdict"] != "OK") \
        .orderBy(F.desc("difference"))
    metric_wrapper = metric_wrapper_func(func.__name__)
    results, legend = per_domain_results(nok_data)
    insert_position = metric_wrapper.find("</div>")
...
def count_rows_per_pidsource_per_domain():
    count_rows_per_pidsource_per_domain = self.data.filter(self.data["name"] == "count_rows_per_pidsource_per_domain") \
        .select("name", "domain", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
    nok_data = count_rows_per_pidsource_per_domain.filter(count_rows_per_pidsource_per_domain["verdict"] != "OK") \
...
def count_unique_panelists_per_dataset():
    count_unique_panelists_per_dataset = self.data.filter(self.data["name"] == func.__name__) \
        .select("name", "domain", "dataset", "value", "expected", "difference", "relativedifference", "threshold", "z_score", "verdict")
    nok_data = count_unique_panelists_per_dataset.filter(count_unique_panelists_per_dataset["verdict"] != "OK") \
        .groupBy(F.desc("difference"))
    metric_wrapper = metric_wrapper_func(func.__name__)
    results, legend = per_domain_per_dataset_results(nok_data)
    insert_position = metric_wrapper.find("</div>")
...
def count_unique_pids_per_domain():
